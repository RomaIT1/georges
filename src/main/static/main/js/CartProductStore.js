class Cart {
   ...
   // ***Змінні
   deliveryPrice = 35
   deliveryFreePrice = 450
   // Об'єкт корзини
   CartStorage = JSON.parse(localStorage.getItem('StorageCart')) ? JSON.parse(localStorage.getItem('CartStorages')) : {
      totalAmount: 0,
   ...
   checkDeliveryPay(){
      if (this.CartStorage.totalPrice >= this.deliveryFreePrice){
   ...
   updateStorage(){
      localStorage.setItem('StorageCart', JSON.stringify(this.CartStorage))
   }
