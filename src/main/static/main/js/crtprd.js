class Cart {
  ...
  // Об'єкт корзини
  CartStorage = JSON.parse(localStorage.getItem(this.StorageName))
  ...
  init() {
    ...
    this.check_promo_state();

    this.update_hour_interval = setInterval(this.update_hour.bind(this), 1000);
    ...
    if (
      this.currentHour < END_WORK_TIME &&
      this.currentHour >= START_WORK_TIME
    ) {
      ...
      this.$orderContainer.forEach(($container, id) => {
        const $orderButtonAdd = $container.querySelector(
          "[data-order-button-add]"
        );
        if ($orderButtonAdd) {
          $orderButtonAdd.addEventListener(
            "click",
            this.addCartProduct.bind(this, id)
          );
        }
      });
      ...
      if (this.$comboForm) {
        ...
        this.$comboForm.addEventListener(
          "submit",
          this.addProductCombo.bind(this)
        );
      }
    } else {
  ...
  addCartProduct(index) {
    ...
    if (this.checkRepeatProduct(productId)) {
      ...
      this.CartStorage.products[
        this.CartStorage.products.indexOf(currentProduct)
      ].amount++;

      // Зміна ціни за товар
      this.CartStorage.products[
        this.CartStorage.products.indexOf(currentProduct)
      ].totalPrice = this.calculateTotalProductPrice(currentProduct);
    ...
    // Якщо такого товару немає в корзині
      ...
      // Деталі товару (name)
      const $orderFeatureName = this.$orderContainer[index].querySelectorAll(
        "[data-order-product-feature-name]"
      );

      // Деталі товару (value)
      const $orderFeatureValue = this.$orderContainer[index].querySelectorAll(
        "[data-order-product-feature-value]"
      );
      ...
      newProduct.name = this.$orderContainer[index].querySelector(
        "[data-order-product-name]"
      ).textContent;
      // Додавання ціни за товар
      newProduct.currentPrice = +this.$orderContainer[index].querySelector(
        "[data-order-product-price]"
      ).textContent;
      // Дадавання ціни загальної
      newProduct.totalPrice = +this.$orderContainer[index].querySelector(
        "[data-order-product-price]"
      ).textContent;
      // Додавання деталей
      newProduct.details = [];
      ...
      newProduct.amount = 1;
      // Додавання картинки
      newProduct.image = this.$orderContainer[index]
        .querySelector("[data-order-product-image]")
        .getAttribute("src");
      ...
      this.CartStorage.totalPrice +=
        this.calculateTotalProductPrice(newProduct);
    }
  ...
  check_promo_state() {
    if (!this.promo.enabled) return;

    if (
      this.currentHour >= this.promo.start.hour &&
      this.currentHour <= this.promo.end.hour &&
      new Date().getDay() >= this.promo.start.day &&
      new Date().getDay() <= this.promo.end.day
    ) {
      for (let trashhold of this.promo.price_thresholds) {
        if (this.CartStorage.totalPrice >= trashhold.after_price) {
          this.CartStorage.totalPromoPrice = Math.round(
            this.CartStorage.totalPrice * trashhold.percent
          );
          ...
          this.CartStorage.totalPromoPrice = 0;
        }
      }
    } else {
    ...
    console.log(this.CartStorage.totalPromoPrice);
  }
  checkDeliveryPay() {
  ...
  renderCartProducts() {
    ...
    this.CartStorage.products.forEach((product) => {
      this.$cartProductsRoot.insertAdjacentHTML(
        "afterbegin",
        `
  ...
  productRemove(index) {
    ...
    const productId =
      this.$cartContainer[index].dataset.cartContainer.length === 4
        ? this.$cartContainer[index].dataset.cartContainer
        : +this.$cartContainer[index].dataset.cartContainer;
    ...
    this.CartStorage.products.splice(
      this.CartStorage.products.indexOf(deleteElement),
      1
    );
  ...
  productSetAmount(index, mode) {
    ...
    if (mode === "+") {
      this.CartStorage.products[
        this.CartStorage.products.indexOf(productSetter)
      ].amount++;
      this.CartStorage.totalAmount++;
      this.CartStorage.totalPrice +=
        this.CartStorage.products[
    ...
    // Якщо зменшуємо кількість товарів
      ...
      // Якщо кількість товару більше 1
        ...
        this.CartStorage.products[
          this.CartStorage.products.indexOf(productSetter)
        ].totalPrice -= productSetter.currentPrice;
      }
  ...
  #addListenerDinamicElement() {
    this.$cartContainer.forEach(($container, id) => {
      const $cartProductButtonRemove = $container.querySelector(
        "[data-cart-product-button-remove]"
      );
      const $cartProductButtonPlus = $container.querySelector(
        "[data-cart-product-button-plus]"
      );
      const $cartProductButtonMinus = $container.querySelector(
        "[data-cart-product-button-minus]"
      );

      $cartProductButtonRemove.addEventListener(
        "click",
        this.productRemove.bind(this, id)
      );
      $cartProductButtonPlus.addEventListener(
        "click",
        this.productSetAmount.bind(this, id, "+")
      );
      $cartProductButtonMinus.addEventListener(
        "click",
        this.productSetAmount.bind(this, id, "-")
      );
    });
  ...
  writeAmountField() {
    this.$cartAmountField.setAttribute(
      "value",
      this.CartStorage.totalPromoPrice
        ? this.CartStorage.totalPromoPrice
        : this.CartStorage.totalPrice
    );
    this.$cartAmountField.value = this.CartStorage.totalPromoPrice
      ? this.CartStorage.totalPromoPrice
      : this.CartStorage.totalPrice;
  }
  writeUserAgentField() {
  ...
  createNameComboProduct() {
    ...
    comboProductName = comboProductName.substring(
      0,
      comboProductName.length - 2
    );
